...
stats.innerHTML = P.stats.map(s => `<div class="stat"><strong>${s.value}</strong><span>${s.label}</span></div>`).join("");

document.getElementById("featuredTitle").textContent = P.featured.title;
document.getElementById("featuredEyebrow").textContent = P.featured.eyebrow;
document.getElementById("featuredDescription").textContent = P.featured.description;
document.getElementById("featuredImg").src = P.featured.image;
document.getElementById("featuredPdf").href = P.featured.pdf;
document.getElementById("featuredTags").innerHTML =
  P.featured.tags.map(t => `<span>${t}</span>`).join("");

const filters = ["All", ...new Set(P.projects.map(p => p.category.split(" / ")[0]))];
...
function renderProjects(filter="All"){
  ...
  const list = filter==="All" ? P.projects : P.projects.filter(p => p.category.startsWith(filter));
  grid.innerHTML = list.map((p, idx) => `
    <article class="project-card">
      ...
      <div class="project-body">
        ...
        <a class="text-link" href="${p.link}" target="_blank" rel="noopener">View project PDF ↗</a>
      </div>
    </article>
  `).join("");
}
...
menuBtn?.addEventListener("click", () => {
  ...
  if(!open){
    nav.style.position="absolute"; nav.style.right="18px"; nav.style.top="65px";
    nav.style.flexDirection="column"; nav.style.alignItems="stretch";
    nav.style.background="#fff"; nav.style.padding="18px";
